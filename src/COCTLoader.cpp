...
void COCTLoader::OCTLoadLights(engine::io::IReadFile* file, engine::scene::ISceneManager * scene, engine::scene::ISceneNode * parent, f32 radius, f32 intensityScale, bool rewind)
{
	...
	for (i = 0; i < header.numLights; i++)
	{
		...
		//engine::scene::ISceneNode* node = scene->addTestSceneNode(30,parent,-1, core::vector3df(lights[i].pos[0], lights[i].pos[2], lights[i].pos[1]));
...
IAnimatedMesh* COCTLoader::createMesh(engine::io::IReadFile* file)
{
