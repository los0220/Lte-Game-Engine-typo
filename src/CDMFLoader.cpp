...
// his permission to add it into engine using the zlib license.
                               ...
                               f32 out[3])                             //Normal computed
{
	...
	f32 dist = (f32)FastSqrt((out[0] * out[0]) + (out[1] * out[1]) + (out[2] * out[2]));
...
IAnimatedMesh* CDMFLoader::createMesh(engine::io::IReadFile* file)
{
//	printf("""Funzione chiamata1\n");
	...
	if (GetDMFHeader(dmfRawFile, header))
	{
		...
		use_mat_dirs=SceneMgr->getParameters()->getParameterAsBool(DMF_USE_MATERIALS_DIRS);
		...
		for (i = 0; i < header.numFaces; i++)
		{
			...
			for (v = 0; v < faces[i].numVerts; v++)
			{
				...
				vert.Color = engine::video::SColor(0,255,255,255);
				vert.Normal.set(normal[0], normal[1], normal[2]);
				if(materiali[faces[i].materialID].textureBlend==4 && SceneMgr->getParameters()->getParameterAsBool(DMF_FLIP_ALPHA_TEXTURES))
				{
...
//		    vert.TCoords2.set(vv->lc[0],vv->lc[1]);
		...
		for (i=0; i<header.numMaterials; i++)
		{
			String path;
			path = "";
			if ( !SceneMgr->getParameters()->getParameter(DMF_TEXTURE_PATH) )
			{
				...
				path = path +
				       String( SceneMgr->getParameters()->getParameter(DMF_TEXTURE_PATH)) + String("\\");
			...
			if(materiali[i].textureFlag==1)
			{
				...
				//just for compatibility with older engine versions
			...
			if(materiali[i].textureBlend==4) {
				buffer->Material.MaterialType = video::EMT_TRANSPARENT_ALPHA_CHANNEL;
				buffer->Material.MaterialTypeParam =SceneMgr->getParameters()->getParameterAsFloat(DMF_ALPHA_CHANNEL_REF);
			}
			...
			//if texture is present mengineor vertically owing to DeleD rapresentation
			if (tex && header.dmfVersion<1.1)
			{
				void* pp = tex->lock();
				if (pp)
			...
			//if lightmap is present mengineor vertically owing to DeleD rapresentation
			if (lig && header.dmfVersion<1.1)
			{
				void* pp = lig->lock();
				if (pp)
